...
LoRA adapters: Downloaded from HuggingFace on first request, cached in memory.

Request format:
{
    "input": {
        "prompt": "What is photosynthesis?",
        "model": "hindi",            # or "english", "btech", etc.
        "max_tokens": 512,
        "temperature": 0.7,
        "stream": false
    }
...
base_model = None
base_tokenizer = None
loaded_adapters = {}  # cache: model_name -> PeftModel
adapter_lock = Lock()
...
def get_adapter(model_name):
    ...
    with adapter_lock:
        ...
        # Cache the adapter (keep max 3 in memory to avoid OOM)
